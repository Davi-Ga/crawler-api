class Saver:
    ...
    def save_proposition_data(cls, data):
        data_saving = {
            "ementa": (
                data["dados"]["ementa"]
                if "ementa" in data["dados"]
                else None if "dados" in data else None
            ),
            "ementaDetalhada": (
            ...
            "uris": {
                ...
                "uriPropPrincipal": (
                    data["dados"]["uriPropPrincipal"]
                    if "uriPropPrincipal" in data["dados"]
                    else None
                ),
                "uriUltimoRelator": (
                    data["dados"]["statusProposicao"]["uriUltimoRelator"]
                    if "statusProposicao" in data["dados"]
                    else None
                ),
            },
        }
        ...
        if "dados" in data:
            data_saving = {
                "nome": (
                    ...
                    if "nomeCivil" in data["dados"]
                    else data["dados"]["nome"]
                ),
                "cpf": data["dados"]["cpf"] if "cpf" in data["dados"] else None,
                "foto": (
                    data["dados"]["ultimoStatus"]["urlFoto"]
                ...
                "nascimento": (
                    ...
                    if "dataNascimento" in data["dados"]
                    else None
                ),
                "local_nascimento": (
                    f"{data['dados']['municipioNascimento']} - {data['dados']['ufNascimento']}"
                    if "municipioNascimento" in data["dados"]
                    else None
                ),
                "escolaridade": (
                    data["dados"]["escolaridade"]
