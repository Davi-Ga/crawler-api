...
def access_page_jurisprudences(
    name: str, driver: uc.Chrome, jurisprudences: List[str]
) -> List[str]:
    pagination_items = get_page(generate_url(name), driver).find_all(
        class_="pagination_pagination-pages-item__RTw7L"
    )
    if pagination_items is not None:
        ...
        while pages <= number_pagination_items:
            ...
            raw = initial_page.find_all(
                class_="search-snippet-base_SearchSnippetBase__sMKry"
            )

            for j in raw:
                jurisprudence_title = j.find(
                    class_="search-snippet-base_SearchSnippetBase-titleLink__ms7sZ"
                )["href"]
                jurisprudence_page = get_page(jurisprudence_title, driver)
                print(jurisprudence_title)

                if (
                    jurisprudence_page.find(class_="error-page_main__kx7NZ") is None
                    or jurisprudence_page.find(class_="LockWall-inner") is None
                ):
                    if jurisprudence_page.find(class_="tabs-link") is not None:
                        tab_page = get_page(
                            str(jurisprudence_page.find_all(class_="tabs-link").pop(1))
                            .split('href="')
                            .pop(1)
                            .split('"')
                            .pop(0),
                        ...
                        if tab_page.find(class_="JurisprudencePage-title") is not None:
                            item = {
                                "title": tab_page.find(
                                    class_="JurisprudencePage-title"
                                ).text,
                                "body": tab_page.find(
                                    class_="JurisprudencePage-content"
                                ).text,
                            }
                        ...
                        if (
                            jurisprudence_page.find(class_="JurisprudencePage-title")
                            is not None
                        ):
                            item = {
                                "title": jurisprudence_page.find(
                                    class_="JurisprudencePage-title"
                                ).text,
                                "body": jurisprudence_page.find(
                                    class_="JurisprudencePage-content"
                                ).text,
                            }
                            print(f'Adicionado com sucesso {item["title"]}')
                    ...
                    print(
                        f"Página não encontrada {jurisprudence_title}, pulando para a próxima jurisprudência"
                    )
                    continue
...
def access_page_procedural_parts(
    name: str, driver: uc.Chrome, parts: List[str]
) -> List[str]:
    pagination_items = get_page(generate_url(name, 1), driver).find_all(
        class_="pagination-item-link"
    )
    if pagination_items is not None:
        number_pagination_items = len(pagination_items)
        print(f"Número de páginas {number_pagination_items}")
        ...
        while pages <= number_pagination_items:
            ...
            for j in raw:
                ...
                if part_page.find(class_="error-page_main__kx7NZ") is None:
                    related_documents_element = part_page.find(
                        class_="RelatedDocuments-list"
                    )
                    if related_documents_element is not None:
                        related_documents = related_documents_element.find_all(
                            class_="RelatedDocuments-link"
                        )
                    else:
                        related_documents = []
                    ...
                    if related_documents:
                        petition = {
                            "title_petition": part_page.find(
                                class_="document-title"
                            ).text,
                            "body_petition": part_page.find(
                                class_="MotionPage-content"
                            ).text
                            or part_page.find(class_="document-content").text,
                        }

                        for related_document in related_documents:
                            decision = related_document.find(
                                class_="RelatedDocuments-group-type"
                            )
                            print(decision.text)
                            if (
                                decision.text == "Decisão"
                                or decision.text == "Sentença"
                                or decision.text == "Acórdão"
                            ):
                                decision_page = get_page(
                                    related_document["href"], driver
                                )
                                sleep(3)
                                if decision_page.find(class_="tabs-link") is not None:
                                    tab_page = get_page(
                                        str(
                                            decision_page.find_all(
                                                class_="tabs-link"
                                            ).pop(1)
                                        )
                                        .split('href="')
                                    ...
                                    if (
                                        tab_page.find(class_="JurisprudencePage-title")
                                        is not None
                                    ):
                                        item = {
                                            "title": tab_page.find(
                                        ...
                                        tab_page.find(class_="DocumentPage-title")
                                        is not None
                                    ):
                                        item = {
                                            "body": tab_page.find(
                                        ...
                                        print(
                                            f"Adicionado com sucesso item que não possui titulo"
                                        )
                                        break

                                    else:
                                        print(
                                            f"Provavelmente atingimos o limite de busca"
                                        )
                                        break

                                elif (
                                    decision_page.find(class_="JurisprudencePage-title")
                                    is not None
                                ):
                                    item = {
                                        "title": decision_page.find(
                                            class_="document-title"
                                        ).text,
                                        "body": decision_page.find(
                                            class_="JurisprudencePage-content"
                                        ).text,
                                    }
                                    print(f'Adicionado com sucesso {item["title"]}')
                    ...
                    print(
                        f"Página não encontrada {part_title}, pulando para a próxima parte processual"
                    )
                    continue
...
get_jurisprudences(
    get_names(
        person_color="Negra", analised_row="COR", wanted_row="NOME", delimiter="|"
    )
)
